	...
	struct decon_dma_buf_data *dma_buf_data)
{
	...
	decon_dbg("decon_calc_plane_offst f:%d, dma:%u, %u, %u\n",
...
{
	decon_info("%s: %ld Mhz\n", __clk_get_name(decon->res.aclk_dispaud_d),
				clk_get_rate(decon->res.aclk_dispaud_d) / MHZ);
	...
	decon_info("%s: %ld Mhz\n", __clk_get_name(decon->res.d_decon0_vclk),
				clk_get_rate(decon->res.d_decon0_vclk) / MHZ);
	decon_info("%s: %ld Mhz\n", __clk_get_name(decon->res.d_decon0_eclk),
				clk_get_rate(decon->res.d_decon0_eclk) / MHZ);
}
...
static int decon_runtime_resume(struct device *dev)
{
	...
	clk_prepare_enable(decon->res.aclk_dispaud_d);
...
static int decon_runtime_suspend(struct device *dev)
{
	...
	clk_disable_unprepare(decon->res.aclk_dispaud_d);
...
irqreturn_t decon_esd_pcd_handler(int irq, void *dev_id)
{
	...
	esd = &decon->esd;
	err_status = gpio_get_value(esd->pcd_gpio);

	if (esd->pcd_irq_active != err_status)
		goto handler_exit;
...
irqreturn_t decon_esd_err_handler(int irq, void *dev_id)
{
	...
	decon_info("esd: level: %d, state: %d, total: %d\n",  err_status, decon->state, decon->esd.esd_total);
...
static int decon_register_esd_funcion(struct decon_device *decon)
{
	...
	unsigned int pcd_irqf_type = IRQF_TRIGGER_FALLING;
	unsigned int esd_irqf_type = IRQF_TRIGGER_FALLING;
	...
	if (gpio_is_valid(gpio)) {
		...
		dsim_info("esd : pcd_active is %d, %s\n" , esd->pcd_irq_active,
	...
	if (gpio_is_valid(gpio)) {
		...
		esd_irqf_type = (flags & OF_GPIO_ACTIVE_LOW) ? IRQF_TRIGGER_FALLING : IRQF_TRIGGER_RISING;
		dsim_info("esd : err_active is %d, %s\n" , esd->err_irq_active,
			(esd_irqf_type == IRQF_TRIGGER_RISING) ? "rising" : "falling");
		ret++;
	}
	...
	if (esd->pcd_irq) {
		if (devm_request_irq(dev, esd->pcd_irq, decon_esd_pcd_handler,
				pcd_irqf_type | IRQF_ONESHOT, "pcd-irq", decon)) {
