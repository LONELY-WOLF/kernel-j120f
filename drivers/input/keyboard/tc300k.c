...
static irqreturn_t tc300k_interrupt(int irq, void *dev_id)
{
	...
	dev_notice(&client->dev, "[TK] %s 0x%02X\n",__func__, key_val);//okga
...
#endif
#ifdef CONFIG_TOUCHKEY_GRIP
			if (data->tsk_ev_val[i].tsk_keycode == KEY_FRONT_SAR) {
...
//return count;	//temp
	...
	if (buff == 1) {
		...
		input_report_key(data->input_dev, KEY_FRONT_SAR, TSK_RELEASE);
		data->grip_event = 0;
		data->sar_enable = false;
	}
...
static ssize_t grip_3rd_key_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tc300k_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	...
	mutex_lock(&data->lock_fac);
	value = read_tc350k_register_data(data, 0x20, TC350K_CH_PER_DATA_OFFSET);
	mutex_unlock(&data->lock_fac);

	return sprintf(buf, "%d\n", value);
}

static ssize_t grip_3rd_key_raw(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tc300k_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	...
	mutex_lock(&data->lock_fac);
	value = read_tc350k_register_data(data, 0x20, TC350K_CH_RAW_DATA_OFFSET);
	mutex_unlock(&data->lock_fac);

	return sprintf(buf, "%d\n", value);
}

static ssize_t grip_4th_key_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tc300k_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	...
	mutex_lock(&data->lock_fac);
	value = read_tc350k_register_data(data, 0x28, TC350K_CH_PER_DATA_OFFSET);
	mutex_unlock(&data->lock_fac);

	return sprintf(buf, "%d\n", value);
}

static ssize_t grip_4th_key_raw(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tc300k_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	...
	mutex_lock(&data->lock_fac);
	value = read_tc350k_register_data(data, 0x28, TC350K_CH_RAW_DATA_OFFSET);
	mutex_unlock(&data->lock_fac);

	return sprintf(buf, "%d\n", value);
}

static ssize_t touchkey_key_code(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct tc300k_data *data = dev_get_drvdata(dev);
	struct i2c_client *client = data->client;
	...
	// if sar_mode is on => must send wake-up command
	if (data->sar_mode) {
		value = tc300k_wake_up(client, TC300K_CMD_WAKE_UP);
	}
#endif

	mutex_lock(&data->lock_fac);
	value = i2c_smbus_read_byte_data(client, TC300K_KEYCODE);
	mutex_unlock(&data->lock_fac);

	return sprintf(buf, "%x\n", value);
}

static DEVICE_ATTR(touchkey_3rd, S_IRUGO, grip_3rd_key_show, NULL);
static DEVICE_ATTR(touchkey_3rd_raw, S_IRUGO, grip_3rd_key_raw, NULL);
static DEVICE_ATTR(touchkey_4th, S_IRUGO, grip_4th_key_show, NULL);
static DEVICE_ATTR(touchkey_4th_raw, S_IRUGO, grip_4th_key_raw, NULL);
static DEVICE_ATTR(touchkey_key_code, S_IRUGO, touchkey_key_code, NULL);
#endif
...
static struct attribute *sec_touchkey_attributes_350k[] = {
	...
	&dev_attr_touchkey_key_code.attr,
...
#if !defined(CONFIG_TOUCHKEY_GRIP)
